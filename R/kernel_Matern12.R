KernelClass_Matern12 <- setRefClass("SqExpKernel",
                                    ...
                                    methods = list(
                                      parainit = function(y,p,B) {
                                        ...
                                        parameters <<- matrix(c(log(1), #sigma
                                                                0, #mu
                                                                seq(1,B), #lambda as B(Z) B(Z) becomes very small
                                      ...
                                      kernel_mat = function(X1,X2,Z1,Z2) {
                                        ...
                                        Klist <- kernmat_Matern12_cpp(X1,X2,Z1,Z2,parameters) #lambda and L

                                      },
                                      kernel_mat_sym = function(X,Z) {
                                        #intended use for the gradient step
                                        Klist <- kernmat_Matern12_symmetric_cpp(X,Z,parameters)
                                        Kmat <<- Klist$full
                                      ...
                                      para_update = function(iter,y,X,Z,Optim,printevery=100) {
                                        ...
                                        gradients <- grad_Matern_cpp(y,X,Z,Kmat,Karray,invKmatn,invKmatList$eigenval,
                                                                     parameters,stats,B,0)
                                        #} else {
                                        #  gradlist <- grad_Matern52_Hessian_cpp(y,X,Z,Kmat,Karray,invKmatn,invKmatList$eigenval,
                                        #                                  parameters,stats,B)
