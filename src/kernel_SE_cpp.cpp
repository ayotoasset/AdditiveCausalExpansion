...
arma::vec grad_SE_cpp(const arma::vec& y,
  ...
  */
  arma::vec gradients(parameters.size()); gradients.zeros();
  ...
  arma::mat dK(n, n); dK.zeros(); dK.diag() += exp(parameters[0]); //dK.diag();
  gradients[0] = evid_grad(tmpK, dK);

  //lambda
  for(unsigned int b=0; b<B; b++){
    //Rcpp::Rcout << "b is: " << b << std::endl; check
    gradients[2+b] = evid_grad(tmpK, K.slice(b));
  }

  //L
  //Rcpp::Rcout << "before: " << gradients << std::endl;
  gradients.rows(2+B,1+B*(px+1)) = evid_scale_gradients(X, tmpK, K, parameters.rows(2+B,1+B*(px+1)), B);
  //Rcpp::Rcout << "after: " << gradients << std::endl;
